...
-----------------------------
Milestone 4 answer:
The official training error decreases as the number of epoch increases, but there is no significant
difference in the validation error. This shows that achieving good performance may not require up to 90 epoch.
"""
...
grader.add_basic_part('3c-0', test3c0, max_points=10, max_seconds=2, description="basic sanity check for the answer of dot product of 2 sparse vectors d1 and d2")
...
#  1. Character Features took longer time to run，because it have larger dataset.
#  2. Although extractCharacterFeatures have training error = 0 (which means it can perfectly fit every example) but
#  having a validation error means it cannot have good performance on a new or unseen data.
############################################################
# over fitting!
### 5a
